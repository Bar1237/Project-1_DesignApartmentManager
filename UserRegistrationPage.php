...
<body>
    ...
    <!--Form to take the values of the new user. If a mistake is made in any of the values user will not have to enter every value again. They are kept.-->
    <div id="box">
    <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
        <br>
        <label for="usernameRegisterID">Username:</label>
        <input type="text" id="usernameRegisterID" name="usernameRegister" placeholder="Username" value="<?php echo isset($_POST['usernameRegister']) ? $_POST['usernameRegister'] : '' ?>" /><br><br>

        <label for="passwordRegisterID">Password:</label>
        <input type="text" id="passwordRegisterID" name="passwordRegister" placeholder="Password" value="<?php echo isset($_POST['passwordRegister']) ? $_POST['passwordRegister'] : '' ?>" /><br><br>
        ...
        <label>Manager:</label>
        <input type="radio" name="isManager" style="margin-left:14px;" value="true" <?php if (isset($_POST['isManager']) && $_POST['isManager'] == 'true')  echo ' checked="checked"';?> /> True
        <input type="radio" name="isManager" style="margin-left:24px;" value="false" <?php if (isset($_POST['isManager']) && $_POST['isManager'] == 'false')  echo ' checked="checked"';?> /> False<br><br>
...
$registerUsername = $registerPassword = $fullName = $isManager = $doorNumber = $email = $phoneNumberOne = $phoneNumberTwo = $gender = $moveInDate = "";
...
if(isset($_POST['register_button'])){
    $registerUsername=test_input($_POST['usernameRegister']);
    ...
    //Checking the phoneNumberTwo variable. If it contains any not allowed characters it will warn the user.
        ...
        echo '</script>';
    //If the username is already taken then it will warn the user.
